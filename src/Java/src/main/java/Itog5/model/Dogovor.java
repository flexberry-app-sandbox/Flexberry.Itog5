...
import Itog5.utils.UUIDConverter;
...
public class Dogovor {
    ...
    @EdmIgnore
    @Converter(converterClass = UUIDConverter.class, name = "Dlitelnost")
    @Convert("Dlitelnost")
    @Column(name = "Длительность", length = 16, unique = true, nullable = false)
    private UUID _dlitelnostid;

    @ManyToOne(optional = false, fetch = FetchType.LAZY)
    @JoinColumn(name = "Dlitelnost", insertable = false, updatable = false)
    private Dlitelnost dlitelnost;
